...
class BasicAdmin extends Controller
{
    ...
    protected function _list($dbQuery = null, $isPage = true, $isDisplay = true, $total = false, $result = [])
    {
        ...
        if (null === $db->getOptions('order')) {
            in_array('sort', $db->getTableFields($db->getTable())) ? $db->order(['sort'=>'desc','id'=>'desc']):$db->order(['id'=>'desc']);
        }
        if ($isPage) {
...
//    /**
//     * 在列表显示之前数据输出之前过滤
//     * @param type $aDataList
//     * @return boolean true 交由父类处理 false 则返回数据列表
//     */
//    protected function _data_filter(&$aDataList) {
//        return true;
//    }
//    
//    /**
//     * 在添加或者保存数据之前 可以根据请求方式区分是显示视图还是保存添加操作
//     * @param type $aItemData 如果是添加该数据会为空 如果是编辑该数据会是当条数据数组
//     * @return boolean true 交由父类处理 false 如果是get方式则自行处理视图输出 如果是post 自行处理保存
//     */
//    protected function _form_filter(&$aItemData) {
//        return true;
//    }
//    
//    /**
//     * 保存或者添加完成数据库操作时候的处理
//     * @param type $result
//     * @return boolean true 交由父类处理 false 自行处理
//     */
